...
def train(epoch, model, loss_func, train_loaders, optimizer, trained_model_dir, wandb, args):
    ...
    for batch_idx, (data1, data2, data3, target) in train_loader_iter:
        ...
        if (epoch == 1) and (batch_idx == 0):
            ...
            save_jpg(data1[0:1, :, :, :], trained_model_dir + f'/sample_in1_b{batch_idx}')
            save_jpg(data2[0:1, :, :, :], trained_model_dir + f'/sample_in2_b{batch_idx}')
            save_jpg(data3[0:1, :, :, :], trained_model_dir + f'/sample_in3_b{batch_idx}')
            save_jpg(target[0:1, :, :, :], trained_model_dir + f'/sample_target_b{batch_idx}')
...
def testing_fun(model, loss_func, test_loaders, outdir, args):
    model.eval()
    ...
    for data1, data2, data3, target in test_loader_iter:
        ...
        with torch.no_grad():
            ...
            if "save_inter" in args.model:
                output, dnet_out, gnet_out = model(data1, data2, data3)
            else:
                output = model(data1, data2, data3)
        ...
        if "save_inter" in args.model:
            save_jpg(dnet_out + data2, f"{outdir}/{Test_Data_name}_dnet_out_data2.jpg")
            save_jpg(gnet_out + data2, f"{outdir}/{Test_Data_name}_gnet_out_data2.jpg")
            save_jpg(dnet_out, f"{outdir}/{Test_Data_name}_dnet_out.jpg")
            save_jpg(gnet_out, f"{outdir}/{Test_Data_name}_gnet_out.jpg")
