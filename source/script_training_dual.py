...
# Load denoise model
denoise_model = globals()['myHDR_detail_only_conv333_fixed']
trained_denoise_model_dir = f"./trained-model-myHDR_detail_only_conv333-kalan_aligned_nosat_conv"
trained_denoise_model_filename = 'trained_model1000.pkl'

denoise_model = denoise_model()
denoise_model.apply(weights_init_kaiming)
denoise_model.cuda()
denoise_model = nn.DataParallel(denoise_model)
denoise_model = model_load(denoise_model, trained_denoise_model_dir, trained_denoise_model_filename)
print(f"[INFO] Load denoise model from {trained_denoise_model_dir}")

start_train = time.time()
save_model_interval = max(1, args.epochs // 10)
for epoch in range(start_epoch + 1, args.epochs + 1):
    start = time.time()
    train_loss = train(epoch, model, denoise_model, loss_func, gate_func, train_loaders, optimizer, trained_model_dir, wandb, args)
...
#)
#
#pth_path = f"/home/jaekim/ws/git/myHDR/models_onnx/model-{args.model}.pth"
#torch.save(model.cpu().module, pth_path)
