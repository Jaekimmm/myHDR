    ...
    def __getitem__(self, index):
        ...
        if os.path.exists(sample_path):
            if self.data_name == 'sice':
                ...
                with h5py.File(sample_path, 'r') as f:
                    data = np.stack((f['IN'][3*3:4*3, 0:1496, :],  # short after gain adjustment
                                     f['IN'][4*3:5*3, 0:1496, :],  # mid after gain adjustment
                                     f['IN'][5*3:6*3, 0:1496, :],  # long after gain adjustment
                                     f['GT'][   :   , 0:1496, :])
                                    , axis=0)
                    if self.map_name is not '':
                        map = np.expand_dims(f[f'MAP_{self.map_name}'][   :   , 0:1496, :], axis=0)
            
            if self.crop_div > 0 : 
                data = self.crop_for_div(data, self.crop_div)
                if self.map_name is not '':
                    map = self.crop_for_div(map, self.crop_div)
            if self.crop_size > 0 : 
                data = self.imageCrop(data, self.crop_size)
                if self.map_name is not '':
                    map = self.imageCrop(map, self.crop_size)
            if self.geometry_aug : 
                data = self.image_Geometry_Aug(data)
                if self.map_name is not '':
                    map = self.image_Geometry_Aug(map)
            
            data = torch.from_numpy(data).float()
            if self.map_name is not '':
                map  = torch.from_numpy(map.squeeze(0)).float()
            else:
                map = torch.zeros_like(data[0, 0, :, :].unsqueeze(0))
        return data, map
...
def train_2gindex(epoch, model, loss_func, gate_func, train_loaders, optimizer, trained_model_dir, wandb, args):
...
def testing_2gindex(model, loss_func, gate_func, test_loaders, outdir, args):
    model.eval()
    verbose = False
    ...
    for data, mask in test_loader_iter:
        ...
        save_tensor_to_img(output_raw, f"{outdir}/{Test_Data_name}_out_raw.png")
        save_tensor_to_img(output,     f"{outdir}/{Test_Data_name}_out_gated_{args.gating}.png")
...
def validation(epoch, model, gate_func, valid_loaders, trained_model_dir, args):
    ...
    for data, mask in valid_loader_iter:
        if args.use_cuda:
            data = data.cuda()
            mask = mask.cuda()
