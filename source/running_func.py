...
class data_loader(data.Dataset):
    def __init__(self, data_dir, crop_size=0, crop_div=8, geometry_aug=False, map=''):
        ...
        #self.denoised_dir = './result-myHDR_detail_only_conv333-kalan_aligned_feat16-e50-'
    ...
    def __getitem__(self, index):
        ...
        #change data_dir to result_dir and replace .h5 to _denoised.h5
        #denoised_path = sample_path.replace(self.data_dir, self.denoised_dir)
        #denoised_path = denoised_path.replace('.h5', '_hdr.h5')
        
        #if os.path.exists(denoised_path):
        #    print(f"[INFO] (data_loader) {denoised_path} exists")
        #    with h5py.File(denoised_path, 'r') as f:
        #        denoised_mid = f['data'][:, :, :]
        #else:
        #    print(f"[INFO] (data_loader) {denoised_path} does not exist")
            
        return data_out, map, exp#, denoised_mid
...
def train(epoch, model, loss_func, gate_func, train_loaders, optimizer, trained_model_dir, wandb, args):
    ...
    #for batch_idx, (data, mask, exp, denoised_mid) in train_loader_iter:
...
def testing_gate(model, loss_func, gate_func, test_loaders, outdir, args):
    model.eval()
    verbose = False
