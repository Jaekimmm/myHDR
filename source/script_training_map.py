...
train_loaders = torch.utils.data.DataLoader(
    data_loader(args.train_data, crop_size=0, geometry_aug=True, map=args.load_gating_map),
...
for epoch in range(start_epoch + 1, args.epochs + 1):
    start = time.time()
    train_loss = train_map(epoch, model, loss_func, gate_func, train_loaders, optimizer, trained_model_dir, wandb, args)
